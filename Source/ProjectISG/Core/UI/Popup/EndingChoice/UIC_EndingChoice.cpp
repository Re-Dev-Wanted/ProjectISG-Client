...
void UUIC_EndingChoice::AppearUI()
{
	...
	// 버튼 바인드
	EndingChoiceView->GetLeaveThisWorldButton()->OnClicked.AddDynamic(this, &ThisClass::UUIC_EndingChoice::OnLeaveButtonClicked);
	EndingChoiceView->GetRemainThisWorldButton()->OnClicked.AddDynamic(this, &ThisClass::UUIC_EndingChoice::OnRemainButtonClicked);
}
...
void UUIC_EndingChoice::MoveToLobby()
{
	...
	UISGGameInstance* ISGGameInstance = EndingChoiceView->GetGameInstance<UISGGameInstance>();
	ISGGameInstance->DestroySessionAndMoveLevel(EndingChoiceModel->GetLobbyLevel());
}
